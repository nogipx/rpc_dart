...
- **[B-27](B-27-a-tcp-syn-builds-an-endpoint.md)** open (round 274) — `RpcHttp2Server._handleConnection` is wired to the accept stream, so a TCP SYN builds the transport, the endpoint and the application's contracts before a byte arrives: 200 silent sockets gave 200 endpoints and 200 constructions, against 0 and 0 on websocket. Not a leak (all released on disconnect) — an unmetered hold. Two candidate fixes written up, both changing behaviour for existing users, so it is the owner's call
