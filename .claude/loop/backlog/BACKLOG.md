...
- **[B-26](B-26-timeout-bounds-the-wait-not-the-body-read.md)** open (round 272) — `bodyReadTimeout` is a `.timeout()` on `readBody()`'s future, which does not cancel the `await for` inside it, so the 408 goes out and the read loop should keep consuming. RPC-14's shape on a path its sweeps never covered, and the twin of what round 272 removed from `_reject`. Read off the code, not measured; the extra probe arm that settles it is written up
