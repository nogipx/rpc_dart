...
- **[B-29](B-29-the-isize-bomb-is-unbounded-on-web.md)** open (round 285) — the ISIZE-wrap gzip bomb (VM: 4 MiB of zeros -> +1873 MiB, 17.5 s) is stopped by `boundedInflate`, which on web is `=> null` because `package:archive` cannot be halted part-way; the limit then runs on `result.length`, after the output exists. And `isize_wrap_bomb_test.dart` is `@TestOn('vm')`, so the platform without the defence is the one the test cannot reach. Established by READING; the dart2js probe uses TIME, not RSS
- **[B-28](B-28-metadata-is-exempt-from-flow-control.md)** open, MEASURED (round 282) — the asymmetry worry was refuted by reading; what is real is worse than predicted. Metadata is never paced (4000 sends, 32 MiB, none blocked, control parks at exactly the window) **and nothing catches it**: these frames land in the per-stream `StreamController`, which is unweighed and uncapped, not in the bounded broadcast. Flow control is the only thing in front of it. Owner decision — both fixes change behaviour on the path every transport shares
- **[B-27](B-27-a-tcp-syn-builds-an-endpoint.md)** open, half discharged (round 275) — `RpcHttp2Server._handleConnection` is wired to the accept stream, so a TCP SYN built the transport, the endpoint and the application's contracts before a byte arrived: 200 silent sockets gave 200 endpoints, against 0 on websocket. The owner chose the deadline and 275 shipped it (`prefaceTimeout`, 30s, 200 -> 0). **What it does NOT buy**: 24 preface bytes take the hold straight back, so it bounds scanners and stuck load balancers, not a determined peer — `pingInterval` is that half and is still null by default. Deferring the construction itself is still open
