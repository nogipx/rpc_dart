...
- **[C-29](C-29-the-real-scope-of-the-stream-limits.md)** rounds 137, 139 off-journal, core and http2 — responder endpoints are PER CONNECTION (so never call this "the server going offline"); the cost that crosses connections is ~33 KiB per parked stream, i.e. ~136 MB per connection at the default; `halfOpenStreamTimeout` covers DISPATCH ONLY; the peer-keyed bookkeeping audit is complete. Imported after round 235
