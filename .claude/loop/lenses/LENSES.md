...
- **[RPC-15](RPC-15-remeasure-own-record.md)** confirmed (211) — re-measure the loop's own record, including what it claims its own FIXES are worth; refines U-21. **Round 396 adds the cheapest form of it: re-read a record's EVIDENCE, not its conclusion.** C-37 counted 13 per-stream collections from each transport's `health()`, and one of them was not in `health()` at all — so that item rested on a reading while the record read as thirteen measurements. The tell is an inventory whose instrument cannot reach every item it lists; the fix is usually to extend the instrument, which is one line and pays the next round too
