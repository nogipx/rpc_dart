...
2. Если пароль не передан — JSON возвращается как есть. Если пароль передан:
   1. `LicensifyPasswordDerivation.generateBundle()` генерирует соль, количество итераций и
      длину ключа. Эти параметры сохраняются, чтобы по паролю всегда получался тот же ключ.
   2. `LicensifyPasswordDerivation.deriveKeyFromBundle(password, bundle)` прогоняет
      PBKDF2-HMAC-SHA256 и выдаёт симметричный ключ нужного размера.
   3. JSON-снимок шифруется Licensify и упаковывается в PASETO `v4.local`.
   4. Параметры PBKDF2 (`salt`, `iterations`, `keyLength`) сериализуются в JSON и добавляются
      в footer токена.
...
```jsonc
{
  "salt": "...",            // base64-соль PBKDF2
  "iterations": 150000,      // количество итераций
  "keyLength": 32            // длина ключа в байтах
}
```
...
2. Репозиторий определяет формат:
   ...
   - `v4.local` токен — footer декодируется, из него берутся соль/итерации/длина ключа.
3. На основе этих параметров PBKDF2 снова выводит ключ из пароля.
...
Ключ нигде не хранится и не передаётся. В бэкапе лежит только шифртекст и параметры PBKDF2.
PBKDF2 детерминированный, поэтому при тех же входных данных он возвращает идентичный ключ,
которым Licensify расшифровывает снимок. Если footer подделан или повреждён, проверка
