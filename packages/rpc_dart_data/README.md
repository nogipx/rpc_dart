...
2. Если пароль не передан — JSON возвращается как есть. Если пароль передан:
   1. `Licensify.generatePasswordSalt()` генерирует криптостойкую соль минимальной длины 16 байт
      (совместимую с PASERK `k4.local-pw`).
   2. `Licensify.encryptionKeyFromPassword()` прогоняет Argon2id с параметрами по умолчанию
      (`memoryCost = 64 MiB`, `timeCost = 2`, `parallelism = 1`) и возвращает симметричный ключ.
   3. JSON-снимок шифруется Licensify и упаковывается в PASETO `v4.local`.
   4. Параметры Argon2 (`salt`, `memoryCost`, `timeCost`, `parallelism`) и идентификатор `kdf`
      сериализуются в JSON и добавляются в footer токена.
...
```jsonc
{
  ...
  "memoryCost": 67108864,      // объём памяти в байтах (64 MiB)
  "timeCost": 2,               // количество проходов Argon2id
  "parallelism": 1,            // уровень параллелизма
  "kdf": "argon2id-v4.local-pw" // идентификатор PBKW схемы
}
```
...
2. Репозиторий определяет формат:
   ...
   - `v4.local` токен — footer декодируется, из него берутся параметры Argon2.
3. На основе этих параметров Argon2 снова выводит ключ из пароля.
4. Licensify расшифровывает шифртекст токена, получается исходный JSON-снимок.
5. Репозиторий очищает текущие коллекции и заливает данные из снимка в хранилище.

### Почему достаточно помнить пароль

Ключ нигде не хранится и не передаётся. В бэкапе лежит только шифртекст и параметры Argon2
(`salt`, `memoryCost`, `timeCost`, `parallelism`). Алгоритм детерминированный, поэтому при тех же
входных данных он возвращает идентичный ключ, которым Licensify расшифровывает снимок. Если footer
подделан или повреждён, проверка
параметров упадёт и импорт не начнётся.
