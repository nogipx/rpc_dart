...
соединения и репозиторий данных. В CLI доступны дополнительные опции:
...
`DataRepository.exportDatabase` возвращает снимок всех коллекций в виде обычного JSON. В нём
содержатся `formatVersion`, `generatedAt` и словарь `collections` со всеми записями. Репозиторий
больше не занимается шифрованием: держите сервис в защищённой среде или шифруйте данные на
уровне прикладного кода, если это требуется политиками безопасности.

### Импорт базы данных

`ImportDatabaseRequest` принимает тот же JSON. По умолчанию новые записи заливаются поверх
существующих. Если указать `replaceExisting: true`, репозиторий очистит отсутствующие коллекции
и удалит старые записи перед импортом, чтобы снимок стал точной копией исходной базы.
...
## Как довести сервис до состояния «поднял и пользуюсь»

`rpc_dart_data` уже даёт контракт, офлайн-очередь, экспорт/импорт и реактивные
стримы. Чтобы воспринимать его как готовую удалённую базу данных, нужно
добавить несколько недостающих кирпичиков:

- **Хранение по умолчанию должно быть персистентным.** Встроенный профайл
  сервера пока запускается с in-memory адаптером; необходимо переключить его на
  файловый Drift/SQLite, создать каталоги и health-check для диска.
- **Журнал изменений должен переживать рестарт.** Сейчас события для
  `watch()` и `sync()` живут в оперативной памяти (`_eventJournal`). Без
  персистентного `ChangeJournal` после перезапуска теряется прогресс курсоров.
- **Запросы должны делегироваться хранилищу.** `list/search/aggregate`
  выполняют фильтрацию в памяти, поэтому без индексов коллекции растут
  линейно. Расширение `DataStorageAdapter` методами `queryRecords`/`queryAggregate`
  и декларация индексов решают эту проблему.
- **Bulk операции должны быть атомарными.** `bulkUpsert` и `bulkDelete`
  пока последовательно применяют записи. Транзакции в адаптере избавят от
  «полу-записанных» состояний.
- **Нужен простой сценарий backup/restore.** Сейчас `exportDatabase` и
  `importDatabase` есть, но нет CLI/HTTP-обвязки и инструкции для эксплуатации.

Расширенный план действий и дорожная карта описаны в документе
«[Готовая удалённая база: план](../../docs/rpc_dart_data/production_readiness.md)».
В нём подробно расписано, как сделать single-node профиль с SQLite журналом,
индексами и встроенными бэкапами без лишней сложности.
