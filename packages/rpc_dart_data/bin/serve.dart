...
Future<void> main(List<String> arguments) async {
  ...
  try {
    ...
    stderr.writeln('Ошибка настроек secure wrap: ${error.message}');
    exitCode = 64;
    return;
  } catch (error, stackTrace) {
    stderr.writeln('Не удалось инициализировать secure wrap: $error');
  ...
  try {
    ...
    stderr.writeln('Ошибка настроек relay: ${error.message}');
    exitCode = 64;
    return;
  } catch (error, stackTrace) {
    stderr.writeln('Не удалось подготовить подключение к relay: $error');
    stderr.writeln(stackTrace);
    exitCode = 64;
    return;
  }
