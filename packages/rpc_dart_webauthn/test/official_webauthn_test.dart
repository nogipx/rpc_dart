...
class OfficialWebAuthnTestVectors {
  ...
  static List<int> get _clientDataNoAttestation => utf8.encode(
    json.encode({
  ...
  static List<int> get _clientDataSelfAttestation => utf8.encode(
    json.encode({
      'type': 'webauthn.create',
      'challenge': 'dGVzdCBjaGFsbGVuZ2U',
      'origin': 'https://webauthn.guide',
  ...
  static WebAuthnRegistrationCredential createCredentialFromVector(
    ...
    return WebAuthnRegistrationCredential(
      ...
      response: WebAuthnRegistrationResponse(
        clientDataJSON: Uint8List.fromList(
          responseData['clientDataJSON'] as List<int>,
        ),
        attestationObject: Uint8List.fromList(
          responseData['attestationObject'] as List<int>,
        ),
      ),
    );
  }
...
void main() {
  group('Официальные W3C WebAuthn тестовые векторы', () {
    test('16.1.1 ES256 Credential with No Attestation', () {
      ...
      final credential = OfficialWebAuthnTestVectors.createCredentialFromVector(
        vector,
      );
      ...
      final attestationObject =
          decode(credential.response.attestationObject) as Map;
    ...
    test('Сравнение структуры attestationObject между векторами', () {
      final noAttestationVector =
          OfficialWebAuthnTestVectors.es256NoAttestation;
      final selfAttestationVector =
          OfficialWebAuthnTestVectors.es256SelfAttestation;

      final noAttCred = OfficialWebAuthnTestVectors.createCredentialFromVector(
        noAttestationVector,
      );
      final selfAttCred =
          OfficialWebAuthnTestVectors.createCredentialFromVector(
            selfAttestationVector,
          );
