...
- CBOR fast decoder (`_FastCborReader`, the production `CborCodec.decode` path) now handles CBOR tags (skips tag, reads tagged value) and the `undefined` simple value (maps to `null`), matching the slow `CborCodec.decodeUnsafe` reader. Previously these threw `FormatException` on the fast path.
- CBOR robustness: the fast decoder reached full parity with the reference `_CborReader`. It now also decodes indefinite-length arrays, maps, text strings and byte strings; IEEE 754 half (16-bit) and single (32-bit) floats; and extended one-byte simple values. Error messages for the break marker and malformed indefinite lengths were aligned between the two readers. Half/single floats and indefinite lengths were added to the reference reader as well, so both decode the same input set.
...
- Added `test/serializers/cbor_parity_test.dart`, a corpus-based parity guard that pins the two readers and two writers together (int boundaries incl. `>2^32`/`>2^53`/negatives, doubles incl. `NaN`/`Infinity`/`-0.0`, unicode/empty strings, empty/non-empty byte arrays, tags, indefinite lengths, deep nesting). Runs on VM and `-p node`. The two CBOR readers/writers were kept separate (the slow pair is the public `encode/decodeUnsafe` surface, the fast pair backs `encode/decode`); unification would have either cost the fast path or changed the unsafe API, so cross-reference comments plus this parity test enforce that they stay in sync.
