...
description: Implement unary, server streaming, client streaming, and bidirectional RPCs with confidence.
---
...
class ChatContract extends RpcResponderContract {
  ...
  void setup() {
    ...
    addBidirectionalStreamMethod<ClientEvent, ChatMessage>(
      methodName: 'LiveChat',
      handler: _liveChat,
    );
  }
...
<Tabs>
  <TabItem label="Deadlines & cancellation">
  ...
  <TabItem label="Backpressure">
    ...
    you compose with `StreamTransformer`s for custom flow control.
  </TabItem>
  <TabItem label="Zero-copy streaming">
...
channels on top of streaming RPCs.
...
- Metrics via `distributor.metrics` (active clients, total messages, throughput).
