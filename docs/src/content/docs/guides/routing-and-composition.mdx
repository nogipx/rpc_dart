...
description: Combine multiple transports, services, and stream utilities into cohesive topologies.
---
...
```dart
final distributor = StreamDistributor<EventNotification>();
...
class NotificationResponder extends RpcResponderContract {
  ...
  @override
  void setup() {
    addServerStreamMethod<SubscribeRequest, EventNotification>(
      methodName: 'Subscribe',
      handler: (request, {context}) {
        return distributor.getOrCreateClientStream(request.clientId);
      },
    );
  }

  void publish(EventNotification notification) {
...
- Combine `StreamDistributor.metrics` with transport diagnostics to monitor fan
  out and subscriber churn.
