...
- Learn about [Core Concepts](/core-concepts) to understand the building blocks.
- Explore the [transport guides](/transports/inmemory) to pick the right channel for each workload.
- Follow the [RPC Call Lifecycle](/guides/rpc-lifecycle) to see how a request flows end to end.
- Dive into [Streaming Patterns](/guides/streaming-patterns) for real-time interactions.
- Master [Context & Metadata](/guides/context-and-metadata) to propagate deadlines, auth, and tracing.

## Need Help?

- Review the [Error Handling guide](/guides/error-handling) for answers to common production questions
