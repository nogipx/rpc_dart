...
`TurnRelayServer` реализует UDP-реле по RFC 5766 на чистом Dart. Листенер
обрабатывает TURN-запросы Allocate, поднимает отдельные relay-сокеты для
клиентов, следит за разрешениями и каналами и преобразует трафик от пиров в
TURN Data или ChannelData кадры. Реализация поставляется в составе
...
- `clientAddress` / `clientPort` — сокет клиента TURN;
...
Этого достаточно для UDP-профиля RFC 5766.

## Ограничения

- Поддерживается только UDP — TCP allocations, TLS и DTLS пока не реализованы.
